...
def save_autoplay_move(piece, start, end, target, move_score, board_score, white_king_threat, black_king_threat, can_move_score, board, game_id, level, promotion=None, promotion_score=0):
    """移動をリストに追加する"""
    move = {
        "game_id": game_id,
    ...
    if(AIMODE==False):
        with open("chess_ver6.json", "a") as file:
            file.write(json.dumps(move) + "\n")
    else:
        with open("ai_chess_ver6.json", "a") as file:
            file.write(json.dumps(move) + "\n")
    ...
    while run:
                    ...
                    if selected:
                        if (row, col) in valid_moves:
                            if ((row, col)==selected):
                                selected = None
                                valid_moves = []
                            piece = board[selected[0]][selected[1]]  # Assign piece here
                            valid_moves = get_valid_moves(selected, board)
                            target, promotion, promotion_score = move_piece(board, selected , (row, col))
                            ...
                            save_autoplay_move(piece, selected , (row, col), target, move_score, board_score, white_king_threat, black_king_threat, can_move_score, board, game_id, level, promotion, promotion_score)
                            if target == "bK":
                                result = "White wins!"
                                game_over = True
                            selected = None
        ...
        if turn == "b" and not game_over:
            ...
            if start is not None and end is not None and is_valid_move(start, end, board):
                ...
                white_king_threat, black_king_threat = evaluate_king_threat(board)
                can_move_score = calculate_can_move_score(board)
                save_autoplay_move(piece, start, end, target, move_score, board_score, white_king_threat, black_king_threat, can_move_score, board, game_id, level, promotion, promotion_score)
                if target == "wK":
                    result = "Black wins!"
                    game_over = True
                turn = "w"
